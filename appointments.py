...
ALL_BUERGERAMTS = (
    ...
    122254,122271,122273,122277,122280,122282,122284,122291,122286,122296,
    150230,122301,122297,122294,122312,122304,122311,122309,122281,122279,
    122276,122274,122267,122246,122251,122257,122208,122226
)


ANMELDUNG_SERVICE_ID = 324269
...
def get_appointment_dates(buergeramt_ids=ALL_BUERGERAMTS, service_id=ANMELDUNG_SERVICE_ID):
    ...
    if(len(month_widgets) == 0):
        beepy.beep('robot_error')
...
def observe(limit, polling_delay):
    ...
    while duration < limit:
        ...
        print('Getting available appointments.')
        log_appointment_dates(get_appointment_dates())
        print('Done, spleeping for {} seconds', polling_delay)
